...
const CodeBlock = ({ children, className, ...props }: CodeBlockProps) => {
  ...
  // Move useState to top level to avoid conditional hook calls
  ...
  if (mappedLanguage && supportedLanguages.includes(mappedLanguage)) {
    return (
      <SyntaxHighlighter
        ...
        PreTag="div"
        className="!bg-[#1e1e1e] !my-6 !p-4 !border !border-[#3e3e42] rounded-lg !text-sm"
      >
        {String(children).replace(/\n$/, "")}
      </SyntaxHighlighter>
    );
...
export const mdxComponents = {
  ...
  code: CodeBlock,
  pre: (props: BaseProps) => <div {...props} />,
  blockquote: (props: BaseProps) => (
    <blockquote
  ...
  strong: (props: BaseProps) => (
    <strong className="font-bold text-[#dcdcaa]" {...props} />
  ),
  em: (props: BaseProps) => <em className="text-[#ce9178] italic" {...props} />,
  hr: (props: BaseProps) => <hr className="my-8 border-[#3e3e42]" {...props} />,
  table: (props: BaseProps) => (
    <div className="bg-[#1e1e1e] my-6 border border-[#3e3e42] rounded-lg overflow-x-auto">
      <table className="w-full min-w-full border-collapse" {...props} />
    </div>
  ),
  thead: (props: BaseProps) => (
    <thead className="top-0 z-10 sticky bg-[#2d2d30]" {...props} />
  ),
  tbody: (props: BaseProps) => <tbody {...props} />,
  tr: (props: BaseProps) => (
    <tr
